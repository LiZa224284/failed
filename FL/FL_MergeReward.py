...
class CustomAIRLAlgorithm(DensityAlgorithm):
    def __init__(self, *args, failed_demonstrations: Optional[base.AnyTransitions] = None, exp_k, new_r_w, old_r_w, **kwargs):
        self.exp_k = exp_k
    ...
    def __call__(
        ...
        for idx, (obs, act, next_obs) in enumerate(zip(state, action, next_state)):
            ...
            if self.is_stationary:
                ...
                rew = self.new_r_w * (self._regression_models[None].predict(scaled_padded_trans) - self._failed_regression_models[0].predict(scaled_padded_trans)) - 100 + self.old_r_w * old_rews
            else:
                ...
                if time >= len(self._regression_models):
                    ...
                    rew = (self._regression_models[None].predict(scaled_padded_trans) - self._failed_regression_models[0].predict(scaled_padded_trans)) - 100 + old_rews
            rew_list.append(rew)
...
def parse_args():
    ...
    parser.add_argument('--new_r_w', type=float, default=1.0, help='Exponent factor for weighting in kernel regression.')
    parser.add_argument('--old_r_w', type=float, default=1.0, help='Exponent factor for weighting in kernel regression.')
...
if __name__ == "__main__":
    ...
    algorithm_name = f"KR_SFD_exp{args.exp_k}_bw{args.kernel_bandwidth}"
    env_name = 'FL'
    exp_k = args.exp_k
    new_r_w = args.new_r_w
    old_r_w = args.old_r_w
    ...
    expert = model.load("/home/yuxuanli/failed_IRL_new/FL/models/FL_TD3_success.zip")
    ...
    model.save("/home/yuxuanli/failed_IRL_new/FL/models/FL_TD3_merge_success")
