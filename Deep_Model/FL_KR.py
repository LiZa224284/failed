...
class CustomAIRLAlgorithm(DensityAlgorithm):
    def __init__(self, *args, failed_demonstrations: Optional[base.AnyTransitions] = None, exp_k, kr_model=None, **kwargs):
        self.exp_k = exp_k
        ...
        for idx, (obs, act, next_obs) in enumerate(zip(state, action, next_state)):
            ...
            if self.is_stationary:
                ...
                with torch.no_grad():
                    ...
                    rew = kr_reward - 10  # Apply offset to KR model prediction
...
if __name__ == "__main__":
    ...
    algorithm_name = f"Deep_KR_SFD_exp{args.exp_k}_bw{args.kernel_bandwidth}"
    env_name = 'FL'
    exp_k = args.exp_k
