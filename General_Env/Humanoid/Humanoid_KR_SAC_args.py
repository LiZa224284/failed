...
class CustomReacherEnv(gym.Wrapper):
    def __init__(self, env, success_threshold=0.1):
        ...
        self.success_threshold = success_threshold  # 成功的距离阈值

    def step(self, action):
        obs, reward, termination, truncation, info = self.env.step(action)
        
        # 获取机械臂末端的位置和目标位置
        target_id = mujoco.mj_name2id(self.env.unwrapped.model, mujoco.mjtObj.mjOBJ_BODY, "target")
        end_effector_id = mujoco.mj_name2id(self.env.unwrapped.model, mujoco.mjtObj.mjOBJ_BODY, "fingertip")

        target_position = self.env.unwrapped.data.xpos[target_id]
        end_effector_position = self.env.unwrapped.data.xpos[end_effector_id]
    ...
    def __call__(
        ...
        for idx, (obs, act, next_obs) in enumerate(zip(state, action, next_state)):
            ...
            if self.is_stationary:
                ...
                rew = ((self._regression_models[None].predict(scaled_padded_trans) - self._failed_regression_models[0].predict(scaled_padded_trans)))
            else:
...
def parse_args():
    ...
    parser.add_argument('--n_iterations', type=int, default=2000, help='Number of training iterations.')
    parser.add_argument('--exp_k', type=float, default=1.0, help='Exponent factor for weighting in kernel regression.')
    parser.add_argument('--kernel_bandwidth', type=float, default=0.5, help='Bandwidth for the kernel in kernel regression.')
    parser.add_argument('--n_s_demos', type=int, default=50, help='Number of training iterations.')
    parser.add_argument('--n_f_demos', type=int, default=50, help='Number of training iterations.')
    return parser.parse_args()

if __name__ == "__main__":
    args = parse_args()
    project = "Humanoid_1030"
    # algorithm_name = 'KR_SFD_exp1_bw01'
    algorithm_name = f"sign_KR_SFD_exp{args.exp_k}_bw{args.kernel_bandwidth}_sdmeo{args.n_s_demos}__fdmeo{args.n_f_demos}"
    ...
    model = SAC("MlpPolicy", env=env, device = 'cuda')
    ...
    rollouts = rollout.rollout(
        expert,
        rollout_env,
        rollout.make_sample_until(min_timesteps=2000, min_episodes=n_s_demos),
        rng=rng,
    )
    failed_rollouts = rollout.rollout(
        failed_expert,
        rollout_env,
        rollout.make_sample_until(min_timesteps=100, min_episodes=n_f_demos),
        rng=rng,
    )
    ...
    model.save("/files1/Yuxuan_Li/failed_demos/Experiments/General_Env/Humanoid/Humanoid_KR_SAC_model.zip")
