...
def parse_args():
    ...
    parser.add_argument('--kernel_bandwidth', type=float, default=0.4, help='Bandwidth for the kernel in kernel regression.')
    parser.add_argument('--n_s_demos', type=int, default=2, help='Number of training iterations.')
    parser.add_argument('--n_f_demos', type=int, default=2, help='Number of training iterations.')
    return parser.parse_args()

if __name__ == "__main__":
    args = parse_args()
    project = "PandaPush_1030"
    algorithm_name = f"TQC_expert_KR_exp{args.exp_k}_bw{args.kernel_bandwidth}_sdmeo{args.n_s_demos}__fdmeo{args.n_f_demos}"
    ...
    rollout_model = TQC("MultiInputPolicy", env, verbose=1, device="cuda")
    # expert = rollout_model.load("/files1/Yuxuan_Li/failed_demos/Experiments/Robots/PandaSlide/PandaSlide-v3.zip", env) 
    checkpoint = load_from_hub(
        repo_id="chencliu/tqc-PandaSlide-v3",
        filename="tqc-PandaSlide-v3.zip",
    )
    expert = model.load(checkpoint, env=rollout_env)
    failed_expert = RandomPolicyModel(policy=None, env=env)

    rollouts = rollout.rollout(
        expert,
        rollout_env,
        rollout.make_sample_until(min_timesteps=100, min_episodes=n_s_demos),
        rng=rng,
    )
    ...
    with open("/files1/Yuxuan_Li/failed_demos/Experiments/Robots/PandaPush/GenerateDemos/ManualControl_SuccessfulDemos.pkl", "rb") as f:
        my_rollouts = pickle.load(f)
    # my_rollouts = cast(Iterable[types.Trajectory], my_rollouts)
    ...
    # demos_path = '/files1/Yuxuan_Li/failed_demos/Experiments/Robots/PandaPickAndPlace/rollouts_PandaPickAndPlace.pkl'
    # with open(demos_path, 'rb') as f:
    #     rollouts = pickle.load(f) #TrajectoryWithRew

    # failed_demos_path = '/files1/Yuxuan_Li/failed_demos/Experiments/Robots/PandaPickAndPlace/rollouts_failed_PandaPickAndPlace.pkl'
    ...
    density_trainer = CustomAIRLAlgorithm(
        ...
        failed_demonstrations=my_rollouts,
        rl_algo=model,
